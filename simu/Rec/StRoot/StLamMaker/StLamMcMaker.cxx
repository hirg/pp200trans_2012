...
void StLamMcMaker::InitOutTree() {
	...
	fOutTree->SetDirectory( fOutFile );
        ...
        fOutTree->Branch("p_x2", &p_x2, "p_x2/D");
        fOutTree->Branch("p_f1", &p_f1, "p_f1/D");
        fOutTree->Branch("p_f2", &p_f2, "p_f2/D");
        fOutTree->Branch("p_f3", &p_f3, "p_f3/D");
        fOutTree->Branch("p_f4", &p_f4, "p_f4/D");
	...
	fOutTree->Branch("a_nK", &a_nK, "a_nK/I");
        ...
        fOutTree->Branch("trig",   trig, "trig[4]/I");
        fOutTree->Branch("trig_sub",   trig_sub, "trig_sub[4]/I");
...
int StLamMcMaker::Make() {
	...
	if( ftrigSimuMaker->isTrigger(370641) ) trig[3]=1; //AJP
        ...
        for( int i=0; i<4; ++i ) trig_sub[i]=0;
	...
	magn = fmuEvent->magneticField();
        ...
        St_g2t_pythia *record_g2t_pythia = (St_g2t_pythia *)geantDstI("g2t_pythia");
        assert(record_g2t_pythia);
        g2t_pythia_st *table_g2t_pythia = record_g2t_pythia->GetTable();
        assert(table_g2t_pythia);

        p_s = table_g2t_pythia->mand_s;
        p_t = table_g2t_pythia->mand_t;
        p_u = table_g2t_pythia->mand_u;
        p_ptHard = table_g2t_pythia->hard_p;
        ...
        p_f1 = particleTable[4].idhep;
        p_f2 = particleTable[5].idhep;
        p_f3 = particleTable[6].idhep;
        p_f4 = particleTable[7].idhep;
	...
	for( int p=0; p<ngtra; ++p ) {
		...
		if( fabs(ptra_nSigp)>3.0 && fabs(ptra_nSigpi)>3.0 ) continue;  // the track is a p or pi
		...
		for ( int n=0; n<ngtra; ++n ) {
			...
			if( fabs(ntra_nSigp)>3.0 && fabs(ntra_nSigpi)>3.0 ) continue;  // the track is a p or pi
			
			if( fabs(ptra_nSigpi)>3.0 && fabs(ntra_nSigpi)>3.0 ) continue;  // at least one pi
